...
export default function CreateCampModal({ isOpen, onClose, onSuccess, initialData, id }: CreateCampModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) {
                await api.put(`/admin/camps/${id}`, payload);
                toast.success("Camp updated successfully");
            } else {
                await api.post("/admin/camps", payload);
                toast.success("Camp hosted successfully");
            }
            onSuccess();
            onClose();
            ...
            console.error("Failed to handle camp:", error);
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={id ? "Edit Camp" : "Host New Camp"}>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700">Camp Title</label>
                    <input
                        required
                        type="text"
                        placeholder="e.g. Summer Adventure Camp 2024"
                        className="w-full bg-gray-50 border-none rounded-lg py-3 px-4 focus:ring-2 focus:ring-amber-200 transition-all text-sm shadow-sm"
                ...
                <div className="space-y-2">
                    ...
                    <textarea
                        ...
                        className="w-full bg-gray-50 border-none rounded-lg py-3 px-4 focus:ring-2 focus:ring-amber-200 transition-all text-sm shadow-sm resize-none"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            rows={4}
                            placeholder='{"stay": ["Tents"], "food": ["All Meals"]}'
                            className="w-full bg-gray-50 border-none rounded-lg py-3 px-4 focus:ring-2 focus:ring-amber-200 transition-all text-sm shadow-sm resize-none"
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="text-amber-600 hover:text-amber-700 text-xs font-bold flex items-center gap-1 bg-amber-50 px-3 py-1.5 rounded-lg transition-colors"
                            >
                        ...
                        {formData.images.map((url: string, index: number) => (
                            <div key={index} className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        type="url"
                                        placeholder="https://example.com/camp-site.jpg"
                                        className="w-full bg-gray-50 border-none rounded-lg py-3 pl-10 pr-4 focus:ring-2 focus:ring-amber-200 transition-all text-sm shadow-sm"
                                        value={url}
                ...
                <div className="pt-4">
                    <button
                        disabled={loading}
                        type="submit"
                        className="w-full bg-amber-600 text-white py-4 rounded-lg font-bold shadow-xl shadow-amber-200 hover:bg-amber-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-lg"
                    >
                        {loading ? (id ? "Updating..." : "Hosting...") : (id ? "Update Camp" : "Host Camp")}
                    </button>
                </div>
            </form>
        </Modal>
